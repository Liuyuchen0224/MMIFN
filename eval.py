...
table in the following format
    ---------------------------------------------------------------------------
    | ID | filename | COG_score | COG_pred  | COG |
    ---------------------------------------------------------------------------
where the COG_score is the continuous regression score from the model
      the COG_pred  is the predicted label (HEC=0, MCI=1)
      the COG       is the true label (HEC=0, MCI=1)
"""
import numpy as np
import collections
...
def cm_2_2(csv_file):
    ...
    # binary classification between MCI=1 and HEC=0
    pred, label = [], []
...
def crossValid_cm(csv_files, stage):
    ...
    fig = plot_confusion_matrix(cm, ['MCI', 'HEC'])
