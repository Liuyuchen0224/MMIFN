...
class MMFNet(nn.Module):
    ...
    def __init__(self,num_classes=2):
        ...
        self.meg_encoder = nn.Sequential(
            BasicBlock(1, 32,dim =2,kernel_size=(1,15), stride=(1,4),padding=(0,7)),
        ...
        self.mriPoolEmbedding = PoolEmbedding3d(16) 
               
        feature = 256
        self.proj = depthwise_projection(in_features=feature,
        ...
        self.classifier = nn.Sequential(
            nn.Linear(256*4096*2, num_classes),
            nn.ReLU()   
        )
        
    def forward(self, meg, mri):
        meg = self.meg_encoder(meg)
        mri = self.mri_encoder(mri)
        meg_patch = self.megPoolEmbedding(meg)
        mri_patch = self.mriPoolEmbedding(mri)
        
        x = torch.cat([meg_patch, mri_patch], dim=1)
        x = x.view(x.size(0), -1)        
